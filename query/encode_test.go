...
/**
 * Tests whether Indirect references are properly serialized or not
 */
func TestValues_ArrayIndexNestedTypes_Pointer(t *testing.T) {
	type AnotherSubNested struct {
		AnotherValue string `url:"d"`
	}
	...
	tests := []struct {
		...
		},
		{
			...
			url.Values{
				...
				// nil value for the pointer leads to this stringified value
				"C[2][anotherSubNested]": {""},
